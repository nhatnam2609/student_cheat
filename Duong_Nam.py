...
# For calculate distance with OpenCV =====================================================================
...
selected_points = [(24, 536), (1863, 1032), (1973, 137), (1016, 17)]

def perspective_transform_image(input_image, selected_points):
    # Use the given image directly
    image = input_image.copy()

    # Sort and order the selected_points
    left_points = sorted(selected_points[:2], key=lambda pt: pt[1])
    right_points = sorted(selected_points[2:], key=lambda pt: pt[1])
    ordered_points = [left_points[0], right_points[0], right_points[1], left_points[1]]

    # Convert ordered points to a numpy array
    pts1 = np.float32(ordered_points)

    # Define the perspective transformed rectangle dimensions
...
def detect(save_img=False):
    source, weights, view_img, save_txt, imgsz, trace, colored_trk, save_bbox_dim, save_with_object_id, save_img, draw= opt.source, opt.weights, opt.view_img, opt.save_txt, opt.img_size, not opt.no_trace, opt.colored_trk, opt.save_bbox_dim, opt.save_with_object_id, opt.save_img, opt.draw
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        #  [164., 116., 111., 116.]]
        # #
        # # Create a tensor for these bounding boxes with confidence 1.0 and class 0
        # additional_bboxes = torch.tensor([[*bbox,  0.9, 0.0] for bbox in bboxes])
        # pred = [torch.cat((additional_bboxes, pred[0]), 0)]
        # #
        # print('pred',pred)
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det):
                ...
                print(det)
                # cho thuật toán tính khoảng cách
                founded_classes={}
                founded_people={}
                q = 0
                h = 0
                ...
                # loop over tracked bounding boxes and their corresponding IDs

                # for bbox, id in zip(bbox_xyxy, identities):
                #     print("Track ID:", id, "Bounding Box:", bbox)
                # #######################################################
                # # Write results
                # print(det)
